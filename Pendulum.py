...
@ti.func
def f(theta0,theta1,omega0,omega1):
    b0 = -(m[0]+m[1])*g*ti.sin(theta0)-m[1]*l[1]*ti.sin(theta0-theta1)*omega1*omega1
    b1 = l[0]*ti.sin(theta0-theta1)*omega0*omega0-g*ti.sin(theta1)
    b = ti.Vector([b0,b1])

    a00 = (m[0]+m[1])*l[0]
    a01 = m[1]*l[1]*ti.cos(theta0-theta1)
    a10 = l[0]*ti.cos(theta0-theta1)
    a11 = l[1]
    A_inv = ti.Matrix([[a11,-a01],[-a10,a00]])/(a00*a11-a01*a10)
    acc = A_inv@b
    return ti.Vector([ omega0 , omega1 , acc[0], acc[1] ]) #[ d(theta1)/dt, d(theta2)/dt, d(omega1)/dt, d(omega2)/dt]  

# 4th order Runge-Kutta to solve ODEs
@ti.kernel
def RK4():
    k1 = f(ys[0],            ys[1],            ys[2],            ys[3])
    k2 = f(ys[0]+dt*k1[0]/2, ys[1]+dt*k1[1]/2, ys[2]+dt*k1[2]/2, ys[3]+dt*k1[3]/2)
    k3 = f(ys[0]+dt*k2[0]/2, ys[1]+dt*k2[1]/2, ys[2]+dt*k2[2]/2, ys[3]+dt*k2[3]/2)
    k4 = f(ys[0]+dt*k3[0],   ys[1]+dt*k3[1],   ys[2]+dt*k3[2],   ys[3]+dt*k3[3])
