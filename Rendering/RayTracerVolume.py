...
array_max_size = 200
ArrayI  = ti.types.vector( array_max_size  ,ti.i32)  # use this type to allocate local memory within a thread
ArrayF  = ti.types.vector( array_max_size  ,ti.f64)
...
INT_MAX = 0x3fffffff
PI      = 3.1415926536
LIGHT,MIRROR,GLASS,MICROFACET,SUBSURFACE   = 0,1,2,3,4
...
Material = ti.types.struct( type = ti.u8,color = vec3,eta = ti.f64, roughness = ti.f64,kd = ti.f64,sigma_t = ti.f64,sigma_s = ti.f64) # type: [0]light [1]diffuse [2]mirror [3]glass
...
air_sigma_t,air_sigma_s = 2.0,1.5
# air_sigma_t,air_sigma_s = 0.0,0.0
...
# in short: an intersection shortens ray
@ti.func
def HitScene(o,d):
    t,triidx,weights = FLT_MAX,-1,vec3(0,0,0)
    s,i = ArrayI([0 for _ in range(ArrayI.n)]),0
    node = nodes[0]
    while True:
...
@ti.func
def SubsurfaceSamplePoint(o,d,objidx):
    num = 0
    prims   = ArrayI([0 for _ in range(ArrayI.n)])
    weights = ArrayF([0.0 for _ in range(ArrayF.n)])
    s,i = ArrayI([0 for _ in range(ArrayI.n)]),0
    node = nodes[0]
    while True:
        if node.start<=node.end:
            for j in range(node.start,node.end+1):
                tri_idx   = tidx[j]
                obj_idx   = tri2obj[tri_idx]
                tri       = triangles[tri_idx]
                this_t,w  = HitTriangle(o,d, points[tri[0]],points[tri[1]],points[tri[2]],FLT_MAX )
                if this_t < FLT_MAX and obj_idx==objidx:
                    prims[num],weights[3*num+0],weights[3*num+1],weights[3*num+2] = tri_idx,w[0],w[1],w[2]
                    num+=1
            if i==0: break
            i-=1
            node = nodes[s[i]]
        ...
        i0,i1 = node.li,node.ri
        n0,n1 = nodes[i0],nodes[i1]
        t0,t1 = HitAABB(o,d,n0.box,FLT_MAX),HitAABB(o,d,n1.box,FLT_MAX)
        if t0>t1: t0,t1,i0,i1 = t1,t0,i1,i0
        if t0 == FLT_MAX:
            if i==0:break
            i-=1
            ...
            if t1<FLT_MAX:
                s[i] = i1
                i+=1
    idx = ti.random(ti.u16)%num
    return num,prims[idx],vec3(weights[3*idx+0],weights[3*idx+1],weights[3*idx+2])
...
@ti.func
def SampleDistanceExp(sigma):
    t,pdf = 0.0,0.0
    t = -ti.log(1-ti.random())/sigma
    pdf = sigma*ti.exp(-sigma*t)
    return t,pdf

@ti.func
def SampleUniformOnSphere():
    phi = ti.random()*2*PI
    cos_theta = ti.random()*2-1
    sin_theta = ti.sqrt(1-cos_theta*cos_theta)
    return vec3(sin_theta*ti.sin(phi),cos_theta,sin_theta*ti.cos(phi)),1/4/PI
...
def Render():
    for i,j in img:
        ...
        # eta,sigma_t,sigma_s = 1.0,0.9,0.5  # refractive index, transmittance of the medium when ray traveals
        sigma_t,sigma_s = air_sigma_t,air_sigma_s  # refractive index, transmittance of the medium when ray traveals

        accumulator,radiance = vec3(1,1,1),vec3(0,0,0)
       
        for _ in range(10):
            volume_t,volume_pdf = SampleDistanceExp(sigma_t)
            hit_t,hit_tri_idx,hit_weights = HitScene(o,d)
           
            # volume pdf == 0 means sigma ==0 ,which means there is no medium
            if volume_t<hit_t and volume_pdf>EPSILON:
                o = o + d * volume_t
                d,dummy = SampleUniformOnSphere()  # rho(wi,wo)/pdf == 1
                accumulator*= ti.exp(-sigma_t*volume_t)*sigma_s/volume_pdf  
            elif hit_tri_idx != -1:
                obj_idx = tri2obj[hit_tri_idx]
                material = materials[obj_idx]
                ...
                if material.type == GLASS:
                    etaI,etaT,in_air = 1.0,material.eta,True   # default : air to glass;  in_air : is ray origin in air
                    if normal.dot(d) > 0: normal, etaI, etaT, in_air = -normal,material.eta,1.0,False  # glass to air
                    if ti.random() < Fresnel(d,normal,etaI,etaT):
                        ...
                        if in_air:  sigma_s,sigma_t = 0.0,0.0
                        else:       sigma_s,sigma_t = air_sigma_s,air_sigma_t
                # if material.type == SUBSURFACE:
                #     # sample r , solve cubic equation  (1/4)x^3+(3/4)*x+(u-1) = 0;  x = e^(-r/3d)
                #     A = material.color.sum()/3
                #     d = 3.5+100*(A-0.33)**4
                #     u = ti.random()
                #     x =  1 - u
                #     for __ in range(20): x -= (x**3+x+(4/3)*(u-1))/(x*x+1)
                #     r = -3*d*ti.log(x)
                #     u = ti.random()
                #     sample = ToWorld( r*vec3(ti.cos(u*2*PI),0,ti.sin(u*2*PI)), normal)
                #     hit_p = o + d * hit_t
                #     o1 = hit_p + normal*100 + sample
                #     d1 = -normal
                #     objidx = tri2obj[hit_tri_idx]
                #     intersect_num,sample_prim,sample_w = SubsurfaceSamplePoint(o1,d1,objidx)
                #     sample_normal_idx = vn_idx[sample_prim]
                #     sample_normal     =  ( normals[sample_normal_idx[0]]*sample_w[0] + normals[sample_normal_idx[1]]*sample_w[1] + normals[sample_normal_idx[2]]*sample_w[2]  ).normalized()
                #     Sp = A*(x**3+x)/(8*PI*d*r)
                #     accumulator*= Sp*PI
                #     o = o+d*hit_t+EPSILON*normal
                #     d,pdf = SampleCosineWeighted(sample_normal)        
        img[i,j]+=radiance
...
def Initialize():
    ...
    mats.append( Material(GLASS,(1,5,8),1.5, 0.01,1.0))
\
