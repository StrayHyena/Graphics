...
class PF:
    ...
    def Sample(ix:Interaction):
        ...
        pdf = PF.F(ix,Wi)
        return Sample(pdf = pdf ,ray = Ray(o=ix.pos,d = Wi.normalized()),value =pdf )

@ti.data_oriented
class VDBGrid:
    def __init__(self,json,majRes=0):
        TransformV3 = lambda v,M : (M@np.array([v[0],v[1],v[2],1]))[:3]
        self.i2w = np.array(json['i2w']).reshape(4, 4).transpose()
        self.w2i = ti.math.mat4(np.linalg.inv(self.i2w))
        self.bg,self.name,self.min,self.max = json['bg'],json['name'],json["min"],json["max"]
        self.ibox = np.array([json['boxmin'],json['boxmax']])
        self.bmin,self.bmax = vec3(TransformV3(self.ibox[0],self.i2w)),vec3(TransformV3(self.ibox[1],self.i2w))
        # self.box = np.array([ TransformV3(self.ibox[0],self.i2w), TransformV3(self.ibox[1],self.i2w) ])
        self.vscale = np.array([self.i2w[0][0],self.i2w[1][1],self.i2w[2][2]])
        dims = json['dims']
        assert len(dims) == 3 and dims[0] == 5 and dims[1] == 4 and dims[2] == 3
        self.int1cnt,self.int2cnt,self.leafcnt = json['int1cnt'],json['int2cnt'],json['leafcnt']
        int1o_  = np.full((self.int1cnt,3),INone)
        int1ci = np.full((self.int1cnt,),INone)  # ci: child index
        int2ci = np.full((self.int1cnt,2**5,2**5,2**5),INone)
        leafci = np.full((self.int2cnt,2**4,2**4,2**4),INone)
        voxels  = np.full((self.leafcnt,2**3,2**3,2**3),self.bg)
        currActive2,currActiveLeaf = 0,0
        root = json['root']
        self.origin = root['origin']
        for i1,int1 in enumerate(root['children']):
            int1o = np.array(int1['origin'])
            int1o_[i1] = int1o
            int1ci[i1] = i1
            for int2 in int1['children']:
                int2o = np.array(int2['origin'])
                c = (int2o - int1o) // 2**(4+3)
                assert c[0]<2**5 and c[1]<2**5 and c[2]<2**5
                int2ci[i1,c[0],c[1],c[2]] = currActive2
                for leaf in int2['children']:
                    leafo = np.array(leaf['origin'])
                    c = (leafo - int2o) // 2 ** 3
                    assert c[0]<2**4 and c[1]<2**4 and c[2]<2**4
                    leafci[currActive2, c[0], c[1], c[2]] = currActiveLeaf
                    for voxel in leaf['children']:
                        voxelidx = np.array(voxel['pos'])
                        c = (voxelidx//64,(voxelidx-voxelidx//64*64)//8,voxelidx%8)
                        assert c[0]<2**3 and c[1]<2**3 and c[2]<2**3
                        voxels[currActiveLeaf, c[0], c[1], c[2]] = voxel['value']
                    currActiveLeaf += 1
                currActive2 += 1
        self.int1o   = ti.Vector.field(3,ti.i32,(self.int1cnt))
        self.int1ci = ti.field(ti.i32,(self.int1cnt))
        self.int2ci = ti.field(ti.i32,(self.int1cnt, 2 ** 5, 2 ** 5, 2 ** 5))
        self.leafci = ti.field(ti.i32,(self.int2cnt, 2 ** 4, 2 ** 4, 2 ** 4))
        self.voxels  = ti.field(ti.f64,(self.leafcnt, 2 ** 3, 2 ** 3, 2 ** 3))
        self.int1o.from_numpy(int1o_)
        self.int1ci.from_numpy(int1ci)
        self.int2ci.from_numpy(int2ci)
        self.leafci.from_numpy(leafci)
        self.voxels.from_numpy(voxels)
        self.majres = vec3i(max(1,majRes))
        self.maj = ti.field(ti.f64,shape=self.majres)
        if majRes>0:self.InitMaj(self.majres)

    @ti.kernel
    def InitMaj(self,res:vec3):
        for i,j,k in self.maj:
            subbmin = tm.mix(self.bmin,self.bmax,vec3(i, j, k) / res)
            subbmax = tm.mix(self.bmin,self.bmax,vec3(i+1, j+1, k+1) / res)
            coordMin = tm.floor(self.World2FloatIndex(subbmin),ti.i32)
            coordMax = tm.ceil(self.World2FloatIndex(subbmax),ti.i32)
            self.maj[i,j,k] = -MAX
            for ii in range(coordMin[0],coordMax[0]+1):
                for jj in range(coordMin[1],coordMax[1]+1):
                    for kk in range(coordMin[2],coordMax[2]+1):
                        self.maj[i,j,k] = max(self.maj[i,j,k],self.AtIndex(vec3i(ii,jj,kk)))

    @ti.func
    def AtIndex(self, coord:vec3i):
        ret = self.bg
        target_coord = ti.floor((coord - self.origin) / (2 ** (3 + 4 + 5)),ti.i32) * (2 ** (3 + 4 + 5)) + self.origin
        int1idx = INone
        for int1i in range(self.int1cnt):
            if (target_coord - self.int1o[int1i]).norm() == 0: int1idx = int1i
        if int1idx != INone:
            currOrigin = self.int1o[int1idx]
            int2idx = self.int1ci[int1idx]
            if int2idx != INone:
                target_coord = (coord - currOrigin) // (2 ** (3 + 4))
                currOrigin += target_coord * (2 ** (3 + 4))
                assert 0<=target_coord[0] < 2 ** 5 and 0<=target_coord[1] < 2 ** 5 and 0<=target_coord[2] < 2 ** 5
                leafidx = self.int2ci[int2idx, target_coord[0], target_coord[1], target_coord[2]]
                if leafidx != INone:
                    target_coord = (coord - currOrigin) // (2 ** 3)
                    currOrigin += target_coord * (2 ** (3))
                    assert 0<=target_coord[0] < 2 ** 4 and 0<=target_coord[1] < 2 ** 4 and 0<=target_coord[2] < 2 ** 4
                    voxelidx = self.leafci[leafidx, target_coord[0], target_coord[1], target_coord[2]]
                    if voxelidx != INone:
                        target_coord = coord - currOrigin
                        assert 0<=target_coord[0] < 2 ** 3 and 0<=target_coord[1] < 2 ** 3 and 0<=target_coord[2] < 2 ** 3
                        ret = self.voxels[voxelidx, target_coord[0], target_coord[1], target_coord[2]]
        return ret

    @ti.func
    def World2FloatIndex(self,worldpos:vec3):
        idxvec4 = self.w2i@ti.math.vec4(worldpos,1)
        return vec3(idxvec4[0],idxvec4[1],idxvec4[2])

    @ti.func
    def At(self,worldpos:vec3):
        idx = self.World2FloatIndex(worldpos)
        idx000 = ti.floor(idx,ti.i32)
        d = idx-idx000.cast(ti.f64)
        c00 = self.AtIndex(idx000+vec3i(0,0,0))*(1-d[0]) +  self.AtIndex(idx000+vec3i(1,0,0))*d[0]
        c01 = self.AtIndex(idx000+vec3i(0,1,0))*(1-d[0]) +  self.AtIndex(idx000+vec3i(1,1,0))*d[0]
        c_z0 = c00 * (1 - d[1]) + c01 * d[1]
        c10 = self.AtIndex(idx000 + vec3i(0, 0, 1)) * (1 - d[0]) + self.AtIndex(idx000 + vec3i(1, 0, 1)) * d[0]
        c11 = self.AtIndex(idx000 + vec3i(0, 1, 1)) * (1 - d[0]) + self.AtIndex(idx000 + vec3i(1, 1, 1)) * d[0]
        c_z1 = c10 * (1 - d[1]) + c11 * d[1]
        return c_z0 * (1 - d[2]) + c_z1 * d[2]
...
class DDAIter:
    ...
    @ti.func
    def Init(self,bmin,bmax,res_,ray):
        hitT = ray.HitAABB(bmin,bmax)
        if hitT!=NOHIT:
            self.dir = ray.d
            self.res = res_
            a = (bmax - bmin) / res_
            for i in range(3):self.d[i] = ti.abs(a[i] / ray.d[i]) if ray.d[i]!=0 else MAX
            self.base_t = 0
            if ray.o[0]<bmin[0] or ray.o[1]<bmin[1] or ray.o[2]<bmin[2] or ray.o[0]>bmax[0] or ray.o[1]>bmax[1] or ray.o[2]>bmax[2]: self.base_t = hitT+a.max()*EPS
            p0 = ray.At(self.base_t) - bmin
            fc0 = p0 / a
            ic0 = tm.floor(fc0,ti.i32)
            for i in range(3):
                if ray.d[i] == 0: continue
                if ray.d[i] > 0:    self.t[i] = (tm.ceil(fc0[i]) - fc0[i]) * a[i] / self.dir[i]
                else:               self.t[i] = (fc0[i] - tm.floor(fc0[i])) * a[i] / ti.abs(self.dir[i])
            self.curr = vec3i(ic0)
        return self
    ...
    @ti.func
    def IsPrevExceed(self):
        ret = False
        if self.prev[0]>=self.res[0] or self.prev[1]>=self.res[1] or self.prev[2]>=self.res[2] or self.prev[0]<0 or self.prev[1]<0 or self.prev[2]<0 :
            ret = True
        return ret
...
@ti.data_oriented
class Volume:
    def __init__(self,vdbjson,camera = Camera(pos=vec3(0,25,100),target=(0,25,0)),maxDepth=51):
        self.maxdepth = maxDepth
        with open(vdbjson, "r", encoding="utf-8") as file:
            jsondata = json.load(file)
        self.T   = VDBGrid(jsondata['temperature'])
        self.rho = VDBGrid(jsondata['density'],64)
        self.bmin,self.bmax = self.rho.bmin,self.rho.bmax
        self.saScale,self.ssScale,self.TScale,self.LeScale = 0.05,0.5,4500,0.5
        self.g   = 0.0
    ...
    def SampleSaSsSn(self,sa,ss):
        assert 0<=sa+ss<=1
        ret,r = 2,ti.random()  # sample Sigma_n
        if r<sa: ret = 0 # sample Sigma_a
        ...
        for i,j in self.img:
            ...
            while depth < self.maxdepth and not terminate :
                ...
                while u == 2:
                    ...
                    maj = self.rho.maj[ddaiter.prev]
                    if maj == 0: continue
                    while True:
                        rayt += -ti.log(1 - ti.random()) / maj  # from ddaiter.prev_t sample t in 【prev voxel】
