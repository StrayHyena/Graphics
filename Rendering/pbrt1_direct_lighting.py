...
MdmNone = Medium(eta=vec3(0),k=vec3(0))
...
class Scene:
    def __init__(self,meshes,furnace_test=False,camera = Camera(pos=vec3(2,0.5,0),target=(0,0.5,0))):
        ...
        self.lfi2globalfi  = ti.field(ti.i32,self.lfn) # 这个值记录着，light face idx 到上面的所有的face idx的映射
    ...
    def DirectLighting(self,ix:Interaction):
        ...
        # if ix.mat.type==BxDF.Type.Lambertian and ti.abs(ix.normal[1]-1.0)<EPS and intersectTest.Valid():
        #     assert intersectTest.mat.Le.sum()>0 or (intersectTest.mat.Le.sum()==0 and wi.dot(intersectTest.normal)>0)
        if ln.dot(wi)<0 and intersectTest.Valid() and intersectTest.fi==self.lfi2globalfi[lfi]: # 没有射到背面并且相交的是同一个三角形
            ...
            light_pdf_area = intersectTest.mat.Le.sum()/self.ltotal_power
            light_pdf = light_pdf_area/(ti.abs(ln.dot(wi))/dist**2) # respect to solid angle
            ret = bxdf_value*ti.abs(ix.normal.dot(wi)) * Li / light_pdf
        if  self.furnace: ret = vec3(0)
        return ret

    # ① 在每一个表面点加上来自光源的直接光照之后，便无需加入表面本身的自发光了。否则会double counting
    #    可以这么理解
    #    对表面点，我会计算来自光源的那部分立体角的积分
    #    之后再计算余下的部分(间接光照),所以要保证这个间接光不能打到光源。不然就重复计算了。
    # ② 由于像镜子，玻璃这些材质的scatter光的方向是离散的。所以它们的DirectLighting总是返回0
    #    对这些材质要特殊处理。用 DeltaBounce来记录当前的光是否是来自这些材质的scattering。
    #    对于这些光，是可以加上表面的自发光的。可以理解成，这些类型不会采样到光源，因此间接光如果击中光源就可以加上,这么做没有double counting
    @ti.kernel
    def Draw(self):
        for i,j in self.img:
            ...
            L,beta,deltaBounce = vec3(0),vec3(1),True #设成True只是为了depth==0时与光源相交时加上ix的Le
            for depth in range(self.maxdepth):
                ix = self.HitBy(ray)
                if not ix.Valid() :
                    L += beta*self.env_Le
                    break
                if deltaBounce:L += beta*ix.mat.Le
                L += beta*self.DirectLighting(ix)
...
def TestCase(name=''):
    ...
    if name=='Cornell': return Scene([
            ...
            Mesh(Utils.Asset('cornell/quad_bottom.obj'),    Utils.MetalLike(Gold,0.1,0.1)),
            ...
            Mesh(Utils.Asset('cornell/sphere.obj'),         Utils.GlassLike(2.)),
            Mesh(Utils.Asset('cornell/bunny.obj'),          Utils.DiffuseLike(0.3,0.6,0.9)),
            Mesh(Utils.Asset('cornell/torus.obj'),          Material(type=BxDF.Type.Specular)),
            Mesh(Utils.Asset('cornell/lightSmall.obj'),     Utils.DiffuseLike(0.9,0.9,0.9,50,50,50)),
        ],False)
    else:  return Scene([
                    Mesh(Utils.Asset('cornell/quad_top.obj'), Utils.DiffuseLike(0.9, 0.9, 0.9)),
                    Mesh(Utils.Asset('cornell/quad_bottom.obj'), Utils.MetalLike(Gold, 0.005, 0.005)),
                    Mesh(Utils.Asset('cornell/quad_left.obj'), Utils.DiffuseLike(0.6, 0, 0)),
                    Mesh(Utils.Asset('cornell/quad_right.obj'), Utils.DiffuseLike(0., 0.6, 0.)),
                    Mesh(Utils.Asset('cornell/quad_back.obj'), Utils.DiffuseLike(0.9, 0.9, 0.9)),
