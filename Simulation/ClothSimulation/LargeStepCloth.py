...
class ImpactZone:
    ...
    class Zone:
        ...
        def Solve(self):
            ...
            for impact in self.impacts:
                for i, vi in enumerate(impact.idx):
                    assert np.linalg.norm(self.xt[self.vid2i[vi]]) == 0 or np.linalg.norm(
                        self.xt[self.vid2i[vi]] - impact.y[i]) == 0
                    self.xt[self.vid2i[vi]] = impact.y[i]
    ...
    def Update(self,X,Y):
        ...
        for i in range(100,300):
            ...
            self.Update_(X,Y)
            npY,npidx,npw,nptype,nptoi,npn,npx,npy = Y.to_numpy(),self.idx.to_numpy(),self.w.to_numpy(),self.type.to_numpy(),self.toi.to_numpy(),self.n.to_numpy(),self.x.to_numpy(),self.y.to_numpy()
            impacts = [ImpactZone.Impact(npidx[i],npw[i],nptype[i],nptoi[i],npn[i],npx[i],npy[i],self.cs.d) for i in range(self.curr_impacts_num[None])]
            if len(impacts)==0:
                if i!=100: print(i,'impact zone success')
                return
            ...
            # 4. solve impact zone
            for zone in zones:
                if len(zone.idx)<3:print('ERR: zone idx size is , ',len(zone.idx))
                opt_y = zone.Solve()
