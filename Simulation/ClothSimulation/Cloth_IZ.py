import numpy as np, polyscope as ps, taichi as ti
...
class Constraint:
    ...
    class Bend:
        def __init__(self,cloth,k):
            self.kd,indices,edge_counter_pts = k*1e-4,[], {(i,j):[]for i,j in cloth.edges_unique} # 这个边链接了哪些三角形
...
class Collision:
    ...
    def quadratic_root(a: ti.f64, b: ti.f64, c: ti.f64):
        ...
        return 1 if 0<=d<EPS20 or ti.abs(a) < EPS12 else count, ti.min(x0, x1), ti.max(x0, x1)
    ...
    def cubic_root(a: ti.f64, b: ti.f64, c: ti.f64, d: ti.f64):
        ...
        if ti.abs(a) < EPS12 : count,x0,x1 = Collision.quadratic_root(b,c,d)
    ...
    @ti.kernel  # X: current position  Y: next position   ACHTUNG 由于cubic solver精度问题，依然可能漏检
            ...
            for i0, i1, i2 in ti.ndrange(ed[0] - st[0] + 1, ed[1] - st[1] + 1, ed[2] - st[2] + 1):
                ...
                for fii in range(self.cellfn[i]):
                    ...
                    if is_continuous:
                        ...
                        # if vi==201 and vi1==75 and vi2==83:print(vi,vi1,vi2,vi3,a,b,c,d,n,r)
...
class Material:
    def __init__(self,cloth):
        ...
        self.bend = Constraint.Bend(cloth, 1e-2)
...
class ImpactZone:
    ...
    class Zone:
        ...
        def TryAddImpact(self,impact):
            for impact_vi in impact.idx:
                if impact_vi in self.idx:
                    ...
                    return True
            return False
        def ci(self,i,x):  # <=0: conform constraint
        ...
        def Solve(self):
            ...
            xt = np.zeros((self.vn, 3))  # target position
            for impact in self.impacts:
                for i, vi in enumerate(impact.idx):xt[self.vid2i[vi]] = impact.y[i]
            xt = xt.flatten()
            cons = [{'fun': lambda x: -self.ci(i,x),'jac': lambda x: -self.gradci(i,x),'type': 'ineq'} for i in range(self.cn)]  # scipy needs ineq con >=0
            return minimize(lambda x:np.mean((x-xt)**2),xt,jac=lambda x:(x-xt)/2, constraints=cons).x.reshape(self.vn,3)
...
class Simulator:
    ...
    def UpdateX(self):
        for i in self.v:
            self.v[i] = (self.y[i]-self.x[i])/self.h
            if self.v[i].norm()>5: self.v[i]=self.v[i].normalized()*5
            self.x[i] += self.v[i]*self.h
        # for i in self.v:print(i,self.v[i].norm())
